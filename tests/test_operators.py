...
def test_sigmoid(a: float, b: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    if a != b:
        epsilon = 1e-6
        assert sigmoid(a) <= sigmoid(b) + epsilon if a < b else sigmoid(a) >= sigmoid(b) - epsilon    
...
@pytest.mark.task0_2
@given(small_floats, small_floats)
def test_neg_more(a: float, b: float) -> None:
    """Write a test that ensures some other property holds for your functions."""
    # TODO: Implement for Task 0.2.
    # Negating a number twice returns the original number
    assert_close(neg(neg(a)), a)
    
    # Negating the sum of two numbers is equal to the sum of their negations
    assert_close(neg(add(a, b)), add(neg(a), neg(b)))
    
    # The product of a number and its negation is always less than or equal to zero
    assert mul(a, neg(a)) <= 0
    
    # The sum of a number and its negation is always zero
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    sum_of_sums = add(sum(ls1), sum(ls2))
    sum_of_elems = sum(addLists(ls1, ls2))
    assert_close(sum_of_sums, sum_of_elems)
